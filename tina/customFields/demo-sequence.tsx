...
// biome-ignore lint/style/useImportType: <Why is importing React a type?>
...
const demoSteps: DemoStep[] = [
  ...
  },
  {
    ...
    title: "Save Controls",
    content:
      "This is where you would normally save your changes. In demo mode, saves are disabled to prevent modifications.",
    target: "save-bar",
    position: "top",
  },
  {
    ...
    content:
      "Here you can navigate between different collections and content types. Try exploring the available collections!",
...
export const DemoIntroSequence: React.FC<DemoIntroSequenceProps> = ({
  ...
  const handleComplete = () => {
    ...
    setTimeout(() => {
      setIsVisible(false); // Actually hide the component
      setDemoSequenceActive(false);
      onComplete?.();
    }, 800);
  };
  ...
  const handleSkip = () => {
    // Immediate unmount when user clicks X - no animation delay
    setIsVisible(false);
    setDemoSequenceActive(false);
    onComplete?.();
  };
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div
          className={`flex items-start justify-between mb-6 transition-all duration-1000 delay-100 ease-out ${
            isAnimating
        ...
        <div
          className={`flex-1 box-border w-full max-w-full overflow transition-all duration-10000 delay-200 ease-out ${
            isAnimating
              ? "opacity-0 translate-y-1"
              : "opacity-100 translate-y-0"
          }`}
        ...
        <div
          className={`flex items-center justify-between mb-6 transition-all duration-500 delay-300 ease-out ${
            isAnimating
        ...
        <div
          className={`flex justify-between items-center space-x-4 transition-all duration-500 delay-400 ease-out ${
            isAnimating
              ? "opacity-60 translate-y-1"
              : "opacity-100 translate-y-0"
          }`}
      ...
      <div
        ...
        style={{
          background: isContentArea
            ? `radial-gradient(ellipse 400px 300px at ${getTargetPosition(
                target
              )}, 
...
export const setDemoSequenceActive = (isActive: boolean) => {
  ...
  window.dispatchEvent(
    new CustomEvent("tina-demo-sequence-change", {
      detail: { isActive },
    })
  );
};
